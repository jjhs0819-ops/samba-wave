...
> **운영 식별자 placeholder**: 본 문서의 `<YOUR_GCP_PROJECT>`, `<YOUR_VM_PUBLIC_IP>` 등은
> 실제 GCP project ID / 외부 IP 로 치환 후 명령 실행. 코드/공개 문서에 식별자 직접 노출 금지
> (public repo leak 차단 정책).
...
gcloud compute instances create samba-wave-vm \
  ...
  --service-account=github-deploy@<YOUR_GCP_PROJECT>.iam.gserviceaccount.com \
...
1. **Cloud SQL 서비스 계정 키 생성**
   ...
   gcloud projects add-iam-policy-binding <YOUR_GCP_PROJECT> \
     --member=serviceAccount:samba-vm-sql@<YOUR_GCP_PROJECT>.iam.gserviceaccount.com \
     --role=roles/cloudsql.client
   gcloud iam service-accounts keys create sa-key.json \
     --iam-account=samba-vm-sql@<YOUR_GCP_PROJECT>.iam.gserviceaccount.com
   ```
...
gcloud compute instances add-access-config samba-wave-vm --access-config-name="External NAT" --address=<YOUR_VM_PUBLIC_IP> ...
```
