...
interface MarketPolicyForm {
  ...
  kreamAdjustDeadbandPct?: number     // 조정 데드밴드 (%) — 미만 차이는 조정 생략
    ...
    const timer = setTimeout(() => {
      ...
      request<{ success: boolean; data: unknown }>(gsUrl)
        .then(res => {
          ...
          // → 브랜드 목록은 res.data.resultList (한 단계만 들어감)
          const outer = res.data as { resultList?: unknown[] } | null
          const list = (outer?.resultList || []) as Record<string, string>[]
  ...
  const loadGsDelivPlaces = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setGsDelivPlaces(list.map(p => ({
          supAddrCd: String(p.supAddrCd || ''),
          label: `${p.addrGbnNm || ''} ${p.baseAddr || ''}`.trim(),
        })))
      }
    } catch { /* 무시 */ }
  }, [gsAccountId])
        ...
        <div>
          ...
          <div style={{ ...card, padding: '1.5rem', marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              ...
              {marketPolicyTab === 'KREAM' && (
                <>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <NumInput value={mp.kreamAdjustDeadbandPct ?? 1.5} onChange={(v) => { setCurrentMarketPolicy({ ...mp, kreamAdjustDeadbandPct: v }); triggerAutoSave() }} style={{ width: '70px' }} suffix="%" />
                    <span style={{ color: c.textMuted, fontSize: '0.72rem' }}>목표가와 현재가 차이가 이 비율 미만이면 조정 생략(헛조정 차단). 마진 하한 미달 복구는 항상 조정</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {marketPolicyTab !== '플레이오토' && marketPolicyTab !== '스마트스토어' && marketPolicyTab !== '11번가' && marketPolicyTab !== '롯데홈쇼핑' && marketPolicyTab !== '신세계몰(전시)' && marketPolicyTab !== '포이즌' && marketPolicyTab !== 'KREAM' && marketPolicyTab !== 'eBay' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {marketPolicyTab === 'GS샵' && (() => {
                ...
                return (
                  <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.5rem', borderTop: `1px solid ${c.border}`, paddingTop: '0.75rem' }}>
                    ...
                    {(() => {
                      // legacy 단일선택(brandCd/brandNm) → brands 배열로 마이그레이션해 표시
