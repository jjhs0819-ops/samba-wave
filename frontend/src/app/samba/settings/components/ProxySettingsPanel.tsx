    ...
    <>
      ...
      <div style={{ ...card, padding: '1.5rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
          <div>
            <div style={{ fontSize: '1rem', fontWeight: 700, color: c.text }}>프록시 / IP 설정</div>
            <p style={{ fontSize: '0.8125rem', color: c.textMuted, margin: '0.25rem 0 0' }}>전송·수집·오토튠에 사용할 IP/프록시를 관리합니다</p>
          </div>
          <button
        ...
        {proxies.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '2rem', color: c.textMuted, fontSize: '0.8125rem' }}>
            등록된 프록시가 없습니다
          </div>
        ) : (
              ...
              {proxies.map((p, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: `1px solid ${c.border}` }}>
                    <td style={{ padding: '0.6rem 0.75rem', color: c.text }}>
                      {isMainIp && <span style={{ color: c.success, marginRight: '4px', fontSize: '0.7rem' }}>●</span>}
                      {p.name}
                    </td>
                    <td style={{ padding: '0.6rem 0.75rem', color: isMainIp ? c.success : c.textMuted, fontFamily: 'monospace', fontSize: '0.75rem' }}>{masked}</td>
                    ...
                    <td style={{ padding: '0.6rem 0.75rem', textAlign: 'center' }}>
                      <span
                        ...
                        style={{
                          ...
                          background: p.enabled ? '#15786a' : c.border,
      ...
      {proxyModalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ ...card, padding: '1.5rem', width: '420px', maxWidth: '90vw' }}>
            <div style={{ fontSize: '0.9375rem', fontWeight: 600, color: c.text, marginBottom: '1rem' }}>
              {proxyEditIdx !== null ? '프록시 수정' : '프록시 추가'}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div>
                <label style={{ fontSize: '0.75rem', color: c.textSub, marginBottom: '4px', display: 'block' }}>이름</label>
              ...
              <div style={{ background: c.surfaceAlt, border: `1px solid ${c.border}`, borderRadius: '8px', padding: '0.75rem' }}>
                <div style={{ fontSize: '0.75rem', color: c.textSub, marginBottom: '0.5rem' }}>프록시 인증 정보 <span style={{ color: c.textMuted }}>(비워두면 메인 IP)</span></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                  <div>
                    <label style={{ fontSize: '0.7rem', color: c.textSub, marginBottom: '2px', display: 'block' }}>Username</label>
                  ...
                  <div>
                    <label style={{ fontSize: '0.7rem', color: c.textSub, marginBottom: '2px', display: 'block' }}>Password</label>
                  ...
                  <div>
                    <label style={{ fontSize: '0.7rem', color: c.textSub, marginBottom: '2px', display: 'block' }}>IP Address</label>
                  ...
                  <div>
                    <label style={{ fontSize: '0.7rem', color: c.textSub, marginBottom: '2px', display: 'block' }}>Port</label>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', color: c.textSub, marginBottom: '6px', display: 'block' }}>용도 (복수 선택 가능)</label>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  {([
                    { key: 'transmit' as ProxyPurpose, label: '전송', color: c.success },
                    { key: 'collect' as ProxyPurpose, label: '수집', color: c.warn },
                    { key: 'autotune' as ProxyPurpose, label: '오토튠', color: '#0f6a5b' },
                  ]).map(({ key, label, color }) => {
                    ...
                    return (
                      <button key={key} onClick={() => toggleProxyPurpose(key)}
                        style={{
                          ...
                          background: active ? `${color}20` : c.surfaceAlt,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <label style={{ fontSize: '0.8125rem', color: c.text }}>활성화</label>
              </div>
            </div>
