...
export default function SambaDashboard() {
  ...
  if (loading && !stats) {
    return <div style={{ padding: '3rem', textAlign: 'center', color: c.textMuted }}>로딩 중...</div>
  }
    ...
    return (
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
        ...
        {gridLines.map(v => {
          ...
          return (
            <g key={v}>
              ...
              <text x={padL - 6} y={y + 4} textAnchor="end" fill={c.textMuted} fontSize="8">{fmtNum(v)}</text>
            </g>
          )
        })}
        {/* X축 라벨 */}
        {monthlyData.map((_, i) => (
          <text key={i} x={getX(i)} y={H - 5} textAnchor="middle" fill={i === month ? c.primary : c.textMuted} fontSize="8" fontWeight={i === month ? 700 : 400}>{i + 1}월</text>
        ))}
        {/* 총매출 선 */}
        <polyline points={totalPoints} fill="none" stroke={c.textMuted} strokeWidth={2} />
        {/* 이행매출 선 */}
        <polyline points={fulfillmentPoints} fill="none" stroke={c.primary} strokeWidth={2} />
        {/* 총매출 점 + 값 */}
        {monthlyData.map((d, i) => {
          const x = getX(i)
          ...
          return (
            <g key={`t-${i}`}>
              <circle cx={x} cy={y} r={3} fill={c.textMuted} />
              {kVal > 0 && <text x={x} y={y - 10} textAnchor="middle" fill={c.textMuted} fontSize="7">{fmtNum(kVal)}</text>}
            </g>
          )
        })}
        {/* 이행매출 점 + 값 */}
        {monthlyData.map((d, i) => {
          ...
          return (
            <g key={`f-${i}`}>
              <circle cx={x} cy={y} r={3} fill={c.primary} />
              {kVal > 0 && <text x={x} y={y - 10} textAnchor="middle" fill={c.primary} fontSize="7" fontWeight={600}>{fmtNum(kVal)}</text>}
            </g>
          )
        })}
      </svg>
  ...
  return (
    <div style={{ color: c.text }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: c.text, marginBottom: '0.25rem' }}>대시보드</h2>
          <p style={{ fontSize: '0.8125rem', color: c.textSub }}>{year}년 · 누적 현황</p>
        </div>
        <p style={{ fontSize: '0.875rem', color: c.textSub }}>{year}년 {month + 1}월 {now.getDate()}일</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${c.border}` }}>
                <th style={{ textAlign: 'left', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>날짜</th>
                <th style={{ textAlign: 'right', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>총매출</th>
                <th style={{ textAlign: 'right', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>이행매출</th>
                <th style={{ textAlign: 'right', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>이행율</th>
            ...
            <tbody>
              {weeklyData.map((d) => (
                <tr key={d.date.toISOString()} style={{ borderBottom: `1px solid ${c.border}` }}>
                  <td style={{ padding: '0.625rem 0', color: c.text }}>{formatShortDate(d.date)}</td>
                  <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>₩{fmtNum(d.totalSale)}</td>
                  <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>₩{fmtNum(d.fulfillmentSale)}</td>
                  <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>{d.rate}%</td>
        ...
        <div style={{ ...card, padding: '1.5rem' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${c.border}` }}>
                <th style={{ textAlign: 'left', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>구분</th>
                <th style={{ textAlign: 'right', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>총매출</th>
                <th style={{ textAlign: 'right', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>이행매출</th>
                <th style={{ textAlign: 'right', padding: '0.625rem 0', color: c.textSub, fontWeight: 500 }}>이행율</th>
              </tr>
            ...
            <tbody>
              <tr style={{ borderBottom: `1px solid ${c.border}` }}>
                <td style={{ padding: '0.625rem 0', color: c.text }}>금월</td>
                <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>₩{fmtNum(thisMonthSales)}</td>
                <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>₩{fmtNum(thisMonthFulfillmentSales)}</td>
                <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>{thisMonthFulfillment}%</td>
              </tr>
              <tr>
                <td style={{ padding: '0.625rem 0', color: c.text }}>전월</td>
                <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>₩{fmtNum(lastMonthSales)}</td>
                <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>₩{fmtNum(lastMonthFulfillmentSales)}</td>
                <td style={{ padding: '0.625rem 0', textAlign: 'right', color: c.text }}>{lastMonthFulfillment}%</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div style={{ ...card, padding: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: c.textSub }}>{year}년 월간 매출액 (단위: 천원)</p>
          </div>
          <div style={{ display: 'flex', gap: '1rem' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.75rem', color: c.textMuted }}>
              <span style={{ width: '12px', height: '2px', background: c.textMuted }} /> 총매출
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.75rem', color: c.primary }}>
              <span style={{ width: '12px', height: '2px', background: c.primary }} /> 이행매출
            </span>
          </div>
        </div>
        {renderLineChart()}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1.5rem' }}>
        ...
        <div style={{ ...card, padding: '1.5rem' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${c.border}` }}>
                <th style={{ textAlign: 'left', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>소싱처</th>
                <th style={{ textAlign: 'right', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>수집</th>
                <th style={{ textAlign: 'right', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>등록</th>
                <th style={{ textAlign: 'right', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>품절</th>
              </tr>
            </thead>
            <tbody>
              {bySource.map((s) => {
                ...
                return (
                  <React.Fragment key={s.source_site}>
                    <tr
                      style={{ borderBottom: `1px solid ${c.border}`, cursor: hasBrands ? 'pointer' : 'default' }}
                      onClick={() => hasBrands && toggleSource(s.source_site)}
                    >
                      <td style={{ padding: '0.5rem 0', color: c.text, display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                        {hasBrands && (
                          <span style={{ fontSize: '0.625rem', color: c.textMuted, display: 'inline-block', transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s' }}>▶</span>
                        )}
                        {s.source_site}
                      </td>
                      <td style={{ padding: '0.5rem 0', textAlign: 'right', color: c.text }}>{fmtNum(s.total)}</td>
                      <td style={{ padding: '0.5rem 0', textAlign: 'right', color: c.text }}>{fmtNum(s.registered)}</td>
                      <td style={{ padding: '0.5rem 0', textAlign: 'right', color: c.textMuted }}>{fmtNum(s.sold_out)}</td>
                    </tr>
                    {isExpanded && s.brands.map((b) => (
                      <tr key={`${s.source_site}-${b.brand}`} style={{ borderBottom: `1px solid ${c.border}`, background: c.surfaceAlt }}>
                        <td style={{ padding: '0.3rem 0 0.3rem 1.25rem', color: c.textMuted, fontSize: '0.8125rem' }}>- {b.brand}</td>
                        <td style={{ padding: '0.3rem 0', textAlign: 'right', color: c.textMuted, fontSize: '0.8125rem' }}>{fmtNum(b.total)}</td>
                        <td style={{ padding: '0.3rem 0', textAlign: 'right', color: c.textSub, fontSize: '0.8125rem' }}>{fmtNum(b.registered)}</td>
                        <td style={{ padding: '0.3rem 0', textAlign: 'right', color: c.textMuted, fontSize: '0.8125rem' }}>{fmtNum(b.sold_out)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                )
              ...
              {bySource.length > 0 && (
                <tr style={{ borderTop: `1px solid ${c.border}` }}>
                  ...
                  <td style={{ padding: '0.5rem 0', textAlign: 'right', color: c.text, fontWeight: 600 }}>{fmtNum(bySource.reduce((a, s) => a + s.total, 0))}</td>
                  <td style={{ padding: '0.5rem 0', textAlign: 'right', color: c.text, fontWeight: 600 }}>{fmtNum(bySource.reduce((a, s) => a + s.registered, 0))}</td>
                  <td style={{ padding: '0.5rem 0', textAlign: 'right', color: c.textMuted, fontWeight: 600 }}>{fmtNum(bySource.reduce((a, s) => a + s.sold_out, 0))}</td>
                </tr>
              )}
              {bySource.length === 0 && (
                <tr><td colSpan={4} style={{ padding: '1.5rem 0', textAlign: 'center', color: c.textMuted }}>데이터 없음</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div style={{ ...card, padding: '1.5rem' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${c.border}` }}>
                <th style={{ textAlign: 'left', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>마켓 / 계정 / 소싱처</th>
                <th style={{ textAlign: 'right', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>판매비중</th>
                <th style={{ textAlign: 'right', padding: '0.5rem 0', color: c.textSub, fontWeight: 500 }}>등록 상품</th>
              </tr>
            </thead>
            <tbody>
              {byMarket.map((m) => {
                    ...
                    {mExpanded && m.accounts.map((a) => {
                      ...
                      return (
                        <React.Fragment key={a.account_id}>
                          {/* 계정 행 */}
                          <tr
                            style={{ borderBottom: `1px solid ${c.border}`, cursor: 'pointer', background: c.surfaceAlt }}
                            onClick={() => toggleMarketAcct(acctKey)}
                          >
                            <td style={{ padding: '0.4rem 0 0.4rem 1.25rem', color: c.textSub, display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.8125rem' }}>
                              <span style={{ fontSize: '0.5625rem', color: c.textMuted, display: 'inline-block', transform: aExpanded ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s' }}>▶</span>
                          ...
                          {aExpanded && a.sources.map((s) => {
                            ...
                            return (
                              <React.Fragment key={s.source_site}>
                                {/* 소싱처 행 */}
                                <tr
                                  style={{ borderBottom: `1px solid ${c.border}`, cursor: hasBrands ? 'pointer' : 'default', background: c.surfaceAlt }}
                                  onClick={() => hasBrands && toggleAcctSource(srcKey)}
                                >
                                  <td style={{ padding: '0.35rem 0 0.35rem 2.5rem', color: c.textSub, display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.8125rem' }}>
                                    {hasBrands && (
                                      <span style={{ fontSize: '0.5rem', color: c.textMuted, display: 'inline-block', transform: sExpanded ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s' }}>▶</span>
                                    )}
                                    {s.source_site}
                                  </td>
                                  <td style={{ padding: '0.35rem 0', textAlign: 'right' }} />
                                  <td style={{ padding: '0.35rem 0', textAlign: 'right', color: c.textSub, fontSize: '0.8125rem' }}>{fmtNum(s.registered)}</td>
                                </tr>
                                {sExpanded && s.brands.map((b) => (
                                  <tr key={`${s.source_site}-${b.brand}`} style={{ borderBottom: `1px solid ${c.border}`, background: c.surfaceAlt }}>
                                    <td style={{ padding: '0.3rem 0 0.3rem 3.75rem', color: c.textMuted, fontSize: '0.75rem' }}>- {b.brand}</td>
                                    <td style={{ padding: '0.3rem 0', textAlign: 'right' }} />
                                    <td style={{ padding: '0.3rem 0', textAlign: 'right', color: c.textMuted, fontSize: '0.75rem' }}>{fmtNum(b.registered)}</td>
                                  </tr>
                                ))}
                              </React.Fragment>
                            )
