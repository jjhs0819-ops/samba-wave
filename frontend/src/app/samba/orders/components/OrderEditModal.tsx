...
export default function OrderEditModal({ open, editingId, form, setForm, onClose, onSubmit }: Props) {
  const c = useTheme() // 훅은 early return 앞에서 호출
  if (!open) return null
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: '16px', padding: '2rem', width: '640px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1rem' }}>
          {[
            ...
            <div key={f.key}>
              ...
              <input type={f.type} style={{ ...makeInputStyle(c), width: '100%', padding: '0.5rem 0.75rem' }}
          ...
          <div>
            ...
            <select style={{ ...makeInputStyle(c), width: '100%', padding: '0.5rem 0.75rem' }} value={form.shipping_company} onChange={e => setForm({ ...form, shipping_company: e.target.value })}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
          ...
          <button onClick={onSubmit} style={{ ...btn('primary', c), padding: '0.625rem 1.25rem', fontSize: '0.875rem' }}>저장</button>
        </div>
      </div>
    </div>
  )
