...
const statusColor = (code: string | null): string => {
  ...
  if (code === 'in_transit') return c.link
  if (code === 'at_pickup' || code === 'information_received') return c.warn
  return c.textSub
}

export default function TrackingModal({
  open, order, onClose }: Props) {
  const c = useTheme()
  ...
  return (
    ...
    >
      <div
        style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: '12px', width: 'min(620px, 95vw)', maxHeight: '85vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
        onClick={e => e.stopPropagation()}
      >
        {/* 헤더 */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: `1px solid ${c.border}` }}>
          <h3 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 600, color: c.text }}>택배 배송조회</h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {externalUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                style={{ fontSize: '0.7rem', padding: '0.2rem 0.55rem', border: `1px solid ${c.border}`, borderRadius: '4px', color: c.textSub, textDecoration: 'none' }}
              >
                택배사 사이트
              </a>
            )}
        ...
        {/* 메타 정보 박스 */}
        <div style={{ padding: '12px 20px', borderBottom: `1px solid ${c.border}`, fontSize: '0.78rem', color: c.text }}>
          <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr', rowGap: '6px', columnGap: '12px' }}>
            <span style={{ color: c.textSub }}>주문번호</span>
            <span style={{ color: c.text }}>{order.order_number || '-'}</span>
            <span style={{ color: c.textSub }}>상품명</span>
            <span style={{ color: c.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{order.product_name || '-'}</span>
            <span style={{ color: c.textSub }}>수령인</span>
            <span style={{ color: c.text }}>{order.customer_name || '-'}</span>
            <span style={{ color: c.textSub }}>운송장번호</span>
            <span style={{ color: c.text }}>
              <span style={{ color: c.text, fontWeight: 600 }}>{carrier || '-'}</span>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '0' }}>
          {loading && (
            <div style={{ padding: '40px 20px', textAlign: 'center', color: c.textMuted, fontSize: '0.85rem' }}>
              조회 중...
            </div>
          )}
          {error && !loading && (
            <div style={{ padding: '24px 20px', color: c.danger, fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>조회 실패: {error}</div>
              {externalUrl && (
                <a
                  href={externalUrl}
                  target="_blank"
                  rel="noreferrer"
                  style={{ fontSize: '0.78rem', color: c.link, textDecoration: 'underline' }}
                >
                  ...
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '20px 1fr', columnGap: '12px', position: 'relative', paddingBottom: i === data.events.length - 1 ? '0' : '14px' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '0.78rem', color: isLast ? c.text : c.textSub, fontWeight: 600 }}>{ev.status || '-'}</span>
                        {ev.location && <span style={{ fontSize: '0.74rem', color: c.textMuted }}>· {ev.location}</span>}
                      </div>
                      {ev.description && (
                        <div style={{ fontSize: '0.74rem', color: c.textMuted, marginTop: '2px', lineHeight: 1.45 }}>{ev.description}</div>
                      )}
                      <div style={{ fontSize: '0.7rem', color: c.textMuted, marginTop: '3px' }}>{fmtTime(ev.time)}</div>
                    </div>
                  </div>
                )
              })}
