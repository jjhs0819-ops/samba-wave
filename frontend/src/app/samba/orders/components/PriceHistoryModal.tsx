...
export default function PriceHistoryModal({
  open, product, history, onClose }: Props) {
  const c = useTheme()
  if (!open) return null
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: '12px', width: 'min(700px, 95vw)', maxHeight: '85vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: `1px solid ${c.border}` }}>
          <h3 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 600, color: c.text }}>가격 / 재고 이력</h3>
        ...
        <div style={{ padding: '12px 20px', borderBottom: `1px solid ${c.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
            <span style={{ fontSize: '0.65rem', padding: '2px 6px', borderRadius: '3px', background: c.accentBg, color: c.text, fontWeight: 600 }}>{product.source_site}</span>
            <span style={{ fontSize: '0.75rem', color: c.textMuted }}>{product.name}</span>
          </div>
          {costPrices.length > 0 && (
            <div style={{ display: 'flex', gap: '20px', fontSize: '0.78rem', flexWrap: 'wrap' }}>
              {isKream && kreamFastMin > 0 && (
                <div><span style={{ color: c.textMuted }}>빠른배송 </span><span style={{ color: c.text, fontWeight: 600 }}>₩ {fmtNum(kreamFastMin)}</span></div>
              )}
              {isKream && kreamGeneralMin > 0 && (
                <div><span style={{ color: c.textMuted }}>일반배송 </span><span style={{ color: c.text, fontWeight: 600 }}>₩ {fmtNum(kreamGeneralMin)}</span></div>
              )}
              {!isKream && (
                <div><span style={{ color: c.textMuted }}>현재가 </span><span style={{ color: c.text, fontWeight: 600 }}>₩ {fmtNum(currentPrice)}</span></div>
              )}
              <div><span style={{ color: c.textMuted }}>최저가 </span><span style={{ color: c.success, fontWeight: 600 }}>₩ {fmtNum(minPrice)}</span>{minEntry && <span style={{ color: c.textMuted, fontSize: '0.68rem' }}> ({fmtShortDate(String(minEntry.date))})</span>}</div>
              <div><span style={{ color: c.textMuted }}>최고가 </span><span style={{ color: c.danger, fontWeight: 600 }}>₩ {fmtNum(maxPrice)}</span>{maxEntry && <span style={{ color: c.textMuted, fontSize: '0.68rem' }}> ({fmtShortDate(String(maxEntry.date))})</span>}</div>
            </div>
          )}
        </div>
        {/* 이력 테이블 */}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${c.border}` }}>
                <th style={{ padding: '8px 16px', textAlign: 'left', color: c.textSub, fontWeight: 500 }}>날짜</th>
                {isKream ? (
                  <>
                    <th style={{ padding: '8px 16px', textAlign: 'right', color: c.textSub, fontWeight: 500 }}>빠른배송(₩)</th>
                    <th style={{ padding: '8px 16px', textAlign: 'right', color: c.textSub, fontWeight: 500 }}>일반배송(₩)</th>
                  </>
                ) : (
                  <th style={{ padding: '8px 16px', textAlign: 'right', color: c.textSub, fontWeight: 500 }}>원가(₩)</th>
                )}
                <th style={{ padding: '8px 16px', textAlign: 'right', color: c.textSub, fontWeight: 500 }}>재고(수량/O/X)</th>
              </tr>
            </thead>
            <tbody>
              {history.map((h, i) => {
                ...
                return (
                  <React.Fragment key={i}>
                    <tr style={{ borderTop: i > 0 ? `1px solid ${c.border}` : 'none', background: c.surfaceAlt }}>
                      <td style={{ padding: '8px 16px', color: c.text, fontWeight: 600, fontSize: '0.78rem' }}>{fmtHistDate(String(h.date))}</td>
                      {isKream ? (
                        <>
                    ...
                    {opts.map((opt, oi) => {
                      ...
                      return (
                        <tr key={oi} style={{ borderTop: `1px solid ${c.border}` }}>
                          <td style={{ padding: '4px 16px 4px 32px', color: c.textMuted, fontSize: '0.73rem' }}>ㄴ {opt.name || `옵션${oi + 1}`}</td>
                          {isKream ? (
                            <>
                          ...
                          <td style={{ padding: '4px 16px', textAlign: 'right', fontSize: '0.73rem', fontWeight: 600, color: soldOut ? c.danger : c.success }}>
                            {stockLabel}
                          </td>
                        </tr>
                      )
