    ...
    try:
        async with get_write_session() as s:
            ...
            async with httpx.AsyncClient(timeout=60) as c:
                ri = await c.get(
                    f"{ec._base_url}/sell/inventory/v1/inventory_item/{sku}", headers=h
                )
                ...
                sold = (
                    max(
                        (int((o.get("listing") or {}).get("soldQuantity", 0) or 0))
                        for o in offers
                    )
                    if offers
                    else 0
                ...
                cur = (
                    (item.get("availability") or {}).get("shipToLocationAvailability")
                    or {}
                ).get("quantity", 0)
                target = want + sold
                print(
                    f"현재 수량 {cur} | 판매됨 {sold} | 주문가능 {max(0, cur - sold)}"
                )
                ...
                item.pop("sku", None)
                ru = await c.put(
                    f"{ec._base_url}/sell/inventory/v1/inventory_item/{sku}",
                    headers=h,
                    json=item,
                )
                print(
                    f"  inventory_item 수정 {ru.status_code}"
                ...
                for o in offers:
                    ...
                    print(
                        f"  offer 수량 {want} → {ro2.status_code}"
                ...
                for o in rc.json().get("offers", []) if rc.status_code == 200 else []:
                    print(
                        f"  확인: 주문가능 {o.get('availableQuantity')} / 상태 {o.get('status')}"
                    )
    finally:
