...
- 옵션 1개(단일)이면 옵션 없이 등록 (루트 stockQuantity만 사용)
- `isSoldOut: true` → `usable: false`, `stockQuantity: 0`
- `_max_stock` 설정 시 옵션별 재고 상한 적용
- 루트 `stockQuantity`는 옵션 합산으로 자동 설정
