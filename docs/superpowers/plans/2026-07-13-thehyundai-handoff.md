...
## 5-1. SSG 전송 사전 정합 (2026-07-14 오전, 커밋 3800accc8·5a317ba00)

원작자 SSG 이슈들(고시 오분류·6005·옵션가격)을 더현대 관점에서 전수 점검:

| 항목 | 결과 |
|---|---|
| 고시그룹 | ★cat1 "레저/스포츠"(어순 반전)가 복합 cat1 목록에 없어 신발/잡화가 sports(기타재화)로 오분류되던 것 수정 + exact 활동그룹의 품목 가로채기 방지 + cat4 지원 + 더현대 어휘 30여 개 보강. 전 카테고리 1,362개 전수 시뮬로 검증 |
| 고시필드 | 치수(mndr "치수"→size_notice→SSG 0000000003/170) + 취급주의(care_instructions 폴백, "참조" 오탐 수정, 250자 컷). 라이브 신발/의류/키즈 3종 고시 실값 생성 확인 |
| 표준카테고리 | 유사도 매칭이 5단 트리 leaf(수식어) 때문에 전패하던 것 폴백 추가(+유아동 가드), "아동운동화→주니어카시트" 오매칭 패널티로 해소. 운영 SSG 트리 4,334개 실측 5/8→7/8. ★기존 소싱처 결과는 stash 왕복 비교로 완전 동일 확인 |
| 전시카테고리(6005) | 매핑 완료 후 SSG 내부 흐름(유사검색+leaf fallback) — 소싱처 무관 확인, 수정 불요 |
| 옵션가격 게이트 | 더현대 다차원 옵션은 조합별 가격 개념 자체가 없음(stckList에 가격 필드 무) = 단일가 정책 실측 확정 — 현 코드 정확, SSG 게이트는 단일차원 uitmDcPrc 상이 시 정상 차단 |
| 이미지 미러 | image.thehyundai.com 은 Referer 없이 200 — 마켓 미러 다운로드 문제없음 |

## 6. 남은 작업 (순서대로)

1. **일괄 배포 (사장님)** — 백엔드: `feat/thehyundai-local` 미배포 커밋 재빌드
   (심야: a04ac7116 prefetch/28ae779a9 고시/ff40f6bd0 브랜드모달+파이프/
   4415300a4 이미지1000px/89927b514 성별 + 오전: 3800accc8 고시정합/
   5a317ba00 카테고리매핑정합). 프론트: myfork main 푸시 → Vercel (브랜드모달 포함).
