...
async def main():
    ...
    try:
        ...
        print(f"  ❌ 실패: ProductNotFoundError가 발생하지 않음. result keys={list(result.keys()) if result else 'empty'}")
        if result:
            print(f"     name={result.get('name')!r} saleStatus={result.get('saleStatus')!r}")
    ...
    if row:
        ...
        try:
            ...
            print(f"  ✅ sid={sid} → name={(result.get('name') or '')[:30]!r}, saleStatus={result.get('saleStatus')!r}, salePrice={result.get('salePrice')}")
        except ProductNotFoundError:
            print(f"  ⚠️ sid={sid} → ProductNotFoundError (오탐 가능성). 실제 GS샵 페이지 직접 확인 필요")
        except Exception as e:
