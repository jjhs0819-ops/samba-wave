...
async def main():
    ...
    try:
        ...
        for r in rows:
            print(
                f"  {r['created_at']} | {r['status']:10s} | {r['site']:10s}/{r['brand']:20s} → {r['target_ids']}"
            )
        ...
        for mt in ("smartstore", "11st", "lotteon"):
            ...
            for r in rows:
                print(
                    f"    {r['source_site']}/{r['brand_name']} → {r['account_label']} excluded={r['excluded']}"
                )
        ...
        for r in rows:
            print(
                f"  {r['market_type']} {r['source_site']}/{r['brand_name']} → {r['account_label']} "
        ...
        for r in rows:
            print(
                f"  {r['hour']} | {r['market_type']:12s} | {r['cnt']:4d}건 | first={r['first_at']} last={r['last_at']}"
            )
        ...
        for r in rows:
            print(
                f"  [{r['status']:10s}] {r['created_at']} | started={r['started_at']} | "
        ...
        for r in rows:
            print(
                f"  {r['job_type']} | created={r['created_at']} started={r['started_at']} | "
        ...
        for r in rows:
            print(
                f"  {r['market_type']:12s} | {r['account_label']:35s} | {r['additional_fields']}"
            )
        ...
        for r in rows:
            print(
                f"  {r['market_type']:12s} | name={r['market_name']!r:30s} | label={r['account_label']!r:30s} | id={r['id']}"
            )
        ...
        for r in rows:
            print(
                f"  {r['created_at']} | {r['market_type']:10s} | {r['source_site']:10s}/{r['brand_name']:20s} → {r['account_label']:30s} excluded={r['excluded']}"
            )
