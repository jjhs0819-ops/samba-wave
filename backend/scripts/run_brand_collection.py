...
class SambaAPIClient:
    ...
    async def brand_scan(
        ...
        result = await self.request(
            "POST", "/samba/collector/brand-scan", json_data=payload
        )
    ...
    async def brand_discover(self, source_site: str, keyword: str):
        ...
        result = await self.request(
            "POST", "/samba/collector/brand-discover", json_data=payload
        )
        if result:
    ...
    async def brand_create_groups(
        self, source_site: str, brand_name: str, categories: list
    ):
        """카테고리별 그룹 생성"""
        payload = {
            "source_site": source_site,
        ...
        result = await self.request(
            "POST", "/samba/collector/brand-create-groups", json_data=payload
        )
    ...
    async def brand_collect_all(
        self, source_site: str, filter_ids: list, keyword: str, brand: str
    ):
        ...
        result = await self.request(
            "POST", "/samba/collector/brand-collect-all", json_data=payload
        )
        if result and "job_id" in result:
            return result["job_id"]
        return None

    async def collect_filter(
        self, filter_id: str, group_index: int = 0, group_total: int = 0
    ):
        ...
        result = await self.request(
            "POST", f"/samba/collector/collect-filter/{filter_id}", params=params
        )
        if result and "job_id" in result:
            return result["job_id"]
        return None
    ...
    async def ai_tag_generate(self, group_ids: list):
        ...
        result = await self.request(
            "POST", "/samba/proxy/ai-tags/generate", json_data=payload
        )
...
async def main():
    ...
    for source_site, brands in COLLECTION_TARGETS.items():
        ...
        for brand_name in brands:
            ...
            # Step 3: 그룹 생성
            filter_ids = await client.brand_create_groups(
                source_site, brand_name, categories
            )
            ...
            if source_site == "LOTTEON":
                ...
                for idx, fid in enumerate(filter_ids):
                    ...
                    if job_id:
                        log(
                            f"    📋 [{idx + 1}/{len(filter_ids)}] Job {job_id[:8]}... 등록"
                        )
            else:
