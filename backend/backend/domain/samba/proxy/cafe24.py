...
class Cafe24Client(BaseProxyClient):
  ...
  async def ensure_token(self) -> str:
    ...
    if not self.refresh_token:
      raise Cafe24ApiError(401, "NO_TOKEN", "access_token과 refresh_token이 모두 없습니다")
  ...
  async def link_product_to_category(
    self, product_no: int, category_no: int, shop_no: int = 1,
  ) -> dict[str, Any]:
    """상품을 카테고리에 연결."""
  ...
  async def update_product(self, product_no: int, payload: dict[str, Any]) -> dict[str, Any]:
    """상품 수정."""
  ...
  async def upload_images(
    self, product_no: int, image_urls: list[str],
  ) -> dict[str, Any]:
    """상품 이미지 등록 (URL 기반).
  ...
  async def register_options(
    self, product_no: int, options: list[dict[str, Any]],
  ) -> dict[str, Any]:
    """상품 옵션 등록.
    ...
    return await self._call_api(
      "POST", f"/products/{product_no}/options",
  ...
  async def update_variant(
    self, product_no: int, variant_code: str, payload: dict[str, Any],
  ) -> dict[str, Any]:
    """품목별 가격/재고 수정."""
    return await self._call_api(
      "PUT", f"/products/{product_no}/variants/{variant_code}",
  ...
  async def stop_selling(self, product_no: int) -> dict[str, Any]:
    ...
    return await self.update_product(product_no, {
      "shop_no": 1,
  ...
  def build_variant_updates(
    ...
    for v in variants:
      ...
      option_key = " / ".join(option_values) if len(option_values) > 1 else (option_values[0] if option_values else "")
