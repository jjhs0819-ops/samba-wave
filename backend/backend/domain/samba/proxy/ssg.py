...
class SSGClient:
    ...
    # ★하드코딩은 폴백일 뿐이고 라이브 계약목록(listBrand)이 우선한다★
    # 플러그인(plugins/markets/ssg.py)이 전송 시마다 get_contracted_brand_map()으로
    # 계정별 라이브 계약목록을 조회해 우선 적용하며, 라이브에 없는 하드코딩 항목은
    # 죽은 brandId 로 간주해 기타(9999999999) 폴백시킨다. 이 dict 는 라이브 조회
    # 실패({}) 시의 최후 폴백 + 상품명 브랜드 제거(remove_brand_from_name)용.
    CONTRACTED_BRANDS: dict[str, tuple[str, str]] = {
        # ⚠️ 2026-09-01 실측: 해당 계정 계약목록에 없음 — 1)의 라이브 우선 로직이 기타 폴백시킨다
        "게스": ("2000002737", "게스"),
        "guess": ("2000002737", "게스"),
        "나이키": ("2000004827", "나이키"),
        "nike": ("2000004827", "나이키"),
        ...
        # 2026-09-01 실측: 라이브 계약 ID 3000065705 (구 2000006059 는 폐기됨)
        "스케쳐스": ("3000065705", "스케쳐스"),
        "skechers": ("3000065705", "스케쳐스"),
