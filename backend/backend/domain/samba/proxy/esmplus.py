...
class ESMPlusClient:
    ...
    # [fork:esm-relay-mode] 릴레이 게이트웨이 경유 모드.
    #   ESM_RELAY_URL 설정 시 → 그쪽(게이트웨이)으로 라우팅 + JWT 미서명(시크릿 불요).
    #   미설정(본진/시크릿 보유자) → 기존 직접 호출. 기본 inert(env 없으면 무변경).
    BASE = os.environ.get("ESM_RELAY_URL") or "https://sa2.esmplus.com"
    _RELAY_MODE = bool(os.environ.get("ESM_RELAY_URL"))
    ...
    def _headers(self) -> dict[str, str]:
        """API 요청 공통 헤더."""
        if self._RELAY_MODE:  # [fork:esm-relay-mode]
            # 릴레이 모드: ESM 시크릿 없음 → JWT 미서명. opaque 릴레이 토큰 + 스코프 헤더.
            # 게이트웨이가 X-ESM-Seller-Id principal 검증 후 실 ESM JWT 를 주입한다.
            return {
                "Authorization": f"Bearer {os.environ.get('ESM_RELAY_TOKEN', '')}",
                "X-ESM-Site": self.site,  # gmarket | auction
                "X-ESM-Seller-Id": self.seller_id,
...
async def resolve_esm_credentials(
    ...
    """
    # [fork:esm-relay-mode] 릴레이 모드: 팀원은 ESM 시크릿이 없다. placeholder 를 반환해
    #   전 호출부의 `if not hosting_id or not secret_key` 가드(~10곳: 마켓플러그인/order/
    #   returns/cs/팩토리)를 통과시킨다. ESMPlusClient 는 _RELAY_MODE 에서 이 값을 서명에
    #   쓰지 않으므로(게이트웨이가 실 ESM JWT 주입) "relay" 더미는 무해.
    if os.environ.get("ESM_RELAY_URL"):
        return "relay", "relay"
