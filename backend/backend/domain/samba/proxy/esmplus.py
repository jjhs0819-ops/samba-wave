...
class ESMPlusClient:
  ...
  SITE_CONFIG: dict[str, dict[str, Any]] = {
    "gmarket": {"siteType": 2, "siteKey": "Gmkt", "ssiPrefix": "G", "label": "지마켓"},
  ...
  async def _call_api(
    ...
    if resp.status_code >= 400 or (body and result_code != 0):
      ...
      logger.error(f"[{label}] API 에러 {method} {path}: {resp.status_code} / resultCode={result_code} / {msg}")
  ...
  async def register_product(self, data: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in site_detail.items():
      if k.lower() == site_key_lower:
        ...
        break
    logger.info(f"[{self.cfg['label']}] 상품 등록 성공: goodsNo={goods_no}, siteGoodsNo={site_goods_no}")
    return {
  ...
  async def update_product(self, goods_no: str, data: dict[str, Any]) -> dict[str, Any]:
  ...
  async def update_sell_status(self, goods_no: str, data: dict[str, Any]) -> dict[str, Any]:
    """판매상태/가격/재고 수정 — PUT /item/v1/goods/{goodsNo}/sell-status"""
    return await self._call_api("PUT", f"/item/v1/goods/{goods_no}/sell-status", data=data)
  ...
  async def update_images(self, goods_no: str, data: dict[str, Any]) -> dict[str, Any]:
    """이미지 수정 — POST /item/v1/goods/{goodsNo}/images"""
    return await self._call_api("POST", f"/item/v1/goods/{goods_no}/images", data=data)
  ...
  async def fetch_category_tree(
    ...
    async def _walk(parent_code: str, path_prefix: str, depth: int = 0) -> None:
      ...
      try:
        data = await self._call_api("GET", f"/item/v1/categories/site-cats/{parent_code}")
        api_calls += 1
    ...
    top_cats = top_data if isinstance(top_data, list) else top_data.get("subCats", [])
    ...
    label = self.cfg["label"]
    logger.info(f"[{label}] 카테고리 트리 수집 완료: {len(result)}개 leaf, API {api_calls}회 호출")
    return result
  ...
  def transform_product(
    ...
    name = market_names.get(cfg["label"]) or market_names.get("G마켓") or market_names.get("옥션") or product.get("name", "")
    ...
    # 재고
    stock = int(product.get("_stock_quantity", 0) or product.get("stock_quantity", 0) or 99)
      ...
      for opt in options_raw:
        ...
        for val in opt_values:
          if isinstance(val, dict):
            ...
            val_price = int(val.get("priceAdjust", 0) or val.get("price_adjust", 0) or 0)
            val_stock = int(val.get("stock", stock) or stock)
            val_sold_out = val.get("isSoldOut", False) or val.get("is_sold_out", False)
          else:
            val_name = str(val)
            val_price = 0
            val_stock = stock
