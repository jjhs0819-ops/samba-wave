...
class LotteonClient:
    ...
    async def get_orders(self, days: int = 7) -> list[dict[str, Any]]:
        ...
        # odSeq 없는 조기 응답 정리 (#708) — getSROrderList가 간혹
        # {"odNo": "...", "fvrList": []} 처럼 odSeq 없이 먼저 보내는 항목이 있음.
        # 키가 (odNo, None)이라 진짜 주문(odNo, "1")과 병합되지 않고 빈 주문으로 새로 생성됨.
        # 같은 odNo에 odSeq 있는 정상 항목이 있으면 흡수, 없고 내용도 비었으면 버림
        # (다음 수집 주기에 odSeq 포함해 정식으로 들어옴 — 실측 30분 간격).
        for _key in [k for k in merged_map if k[1] in (None, "")]:
            _seqless = merged_map.pop(_key)
            _siblings = [
                v for k, v in merged_map.items() if k[0] == _key[0] and k != _key
            ]
            if _siblings:
                for _sib in _siblings:
                    for k, v in _seqless.items():
                        if v in (None, "", [], {}):
                            continue
                        if _sib.get(k) in (None, "", [], {}):
                            _sib[k] = v
            elif any(
                v not in (None, "", [], {}) for k, v in _seqless.items() if k != "odNo"
            ):
                # 형제 없지만 실제 내용이 있는 항목 — 유실 방지 위해 odSeq=1로 복원
                merged_map[(_key[0], "1")] = _seqless
            # else: 형제도 없고 내용도 비어있음 — 유령이므로 버림
    ...
    async def get_cancel_orders(self, days: int = 7) -> list[dict]:
        """최근 N일 취소 클레임 조회 (getCancellationRequestAndComplateList).

        실측(2026-05-19): 이 API의 clmTpCd 파라미터는 롯데 측에서 무시됨.
        문서에도 없는 파라미터이며 CCNL/RETN/EXCH/없음 4가지 호출이 모두 동일 응답.
        따라서 clmTpCd 없이 호출하고, 응답에서 odTypCd=20(주문취소)인 건만 채택.
        """
        ...
        for claim in raw_list:
            ...
            for item in claim.get("itemList") or []:
                # 응답 odTypCd: 20 주문취소 / 50 AS — 취소만 채택
                if str(item.get("odTypCd", "") or "") != "20":
                    continue
                item["odNo"] = od_no
                item["clmNo"] = clm_no
                result.append(item)
    ...
    async def get_exchanges(self, days: int = 7) -> list[dict]:
        ...
        실측(2026-05-19): cancellation API의 clmTpCd=EXCH 호출은 취소 데이터를 반환 →
        교환으로 잘못 적재되는 사고의 원인. exchangeSearch 단일 소스만 신뢰하고
        응답 odTypCd=30(교환) 인 건만 채택.
        """
        start_dt, end_dt = self._datetime_range(days)
        result: list[dict] = []
        seen_clm_keys: set[str] = set()

        # exchangeSearch API (교환접수 단계 — 접수 후 30분 내 건만 조회됨)
        try:
            data = await self._call_api(
                "POST",
            ...
            for claim in raw_list:
                ...
                clm_no = claim.get("clmNo", "")
                for item in claim.get("itemList") or []:
                    # 응답 odTypCd: 10 주문 / 20 취소 / 30 교환 / 31 교환취소 / 40 반품 /
                    # 41 반품취소 / 50 AS — 진짜 교환(30)만 채택
