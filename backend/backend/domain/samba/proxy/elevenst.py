...
class ElevenstClient:
    ...
    async def ship_order(
        ...
        if result_code != "0":
            # -3308: 묶음배송번호 이미 발송처리(같은 dlvNo의 다른 주문과 함께 처리됨),
            # -3309: 이미 배송완료 → 둘 다 목표 상태(발송됨) 도달이라 성공 처리.
            if result_code in ("-3308", "-3309"):
                logger.info(
                    "[11번가] 발송처리: 이미 처리됨(%s) (dlvNo=%s) — 성공 처리",
                    result_code,
                    dlv_no,
                )
                return True
            # -3313: 주문상태가 이미 다른 상태(배송중/취소 등)로 변경 → 발송처리 대상 없음.
            # 공식문서: "발송처리할 목록이 없는 상태". 즉 이 호출은 송장을 등록하지 못한다.
            # (11번가는 이미 배송중 주문의 송장수정 API를 제공하지 않음.) -3313을 성공으로
            # 처리하면 택배사가 빈칸인데 "전송완료"로 표시되는 거짓성공 → 정직하게 실패 처리.
            # 2026-06-05: 직접전달/기존배송중 주문에서 -3313 거짓성공 버그 fix.
    ...
    async def reject_order(
        ...
        Args:
            ord_no:          주문번호
            ord_prd_seq:     주문상품순번
            ord_cn_rsn_cd:   취소사유코드 (2026-07-09 셀러오피스 판매취소 팝업 실측)
                             [판매자 귀책 — 신용점수 차감]
                               06=배송 지연 예상, 07=상품/가격 정보 잘못 입력,
                               08=상품 품절(전체옵션), 09=옵션 품절(해당옵션)
                             [구매자 귀책 — 차감 없음]
                               18=색상/사이즈/주문정보 변경, 19=다른 상품 잘못 주문,
                               20=구매의사 없어짐 (기본값)
                             ※ 구 코드 10은 목록에 없는 폐기 코드 — 11번가 화면에
                               "타사이트 상품주문"으로 표기되던 원인이라 사용 금지.
        ...
        Note:
            구매자 귀책 코드(18/19/20) 외에는 신용점수 -1점 차감.
