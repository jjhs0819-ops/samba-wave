...
class ElevenstClient:
  ...
  async def get_orders(self, start_time: str, end_time: str) -> list[dict[str, Any]]:
    ...
    URL: GET https://api.11st.co.kr/rest/ordservices/complete/{startTime}/{endTime}
    - startTime/endTime 형식: YYYYMMDDhhmm (예: 202501010000)
    - 최대 조회 기간: 7일, 최대 결과: 3,000건
    - 응답 XML: ns2: 네임스페이스 사용, euc-kr 인코딩
    """
    ...
    async with httpx.AsyncClient(timeout=settings.http_timeout_default) as client:
      resp = await client.get(url, headers=headers)
      logger.info(f"[11번가] GET /ordservices/complete/{start_time}/{end_time} → {resp.status_code}")

      if not resp.is_success:
        raise ElevenstApiError(f"주문 조회 HTTP {resp.status_code}: {resp.text[:300]}")
      ...
      logger.debug(f"[11번가] 주문 응답 원문: {txt[:500]}")
      return self._parse_order_xml(txt)

  @staticmethod
  def _parse_order_xml(text: str) -> list[dict[str, Any]]:
    """주문 목록 XML 파싱. ns2: 네임스페이스를 제거하고 처리."""
    try:
      cleaned = re.sub(r"<(/?)ns2:", r"<", text)
      cleaned = re.sub(r' xmlns[^=]*="[^"]*"', "", cleaned)
      root = ET.fromstring(cleaned)
    except ET.ParseError as e:
      logger.warning(f"[11번가] 주문 XML 파싱 실패: {e}")
      return []

    def elem_to_dict(elem: ET.Element) -> Any:
      """XML 엘리먼트를 dict로 재귀 변환. 동일 태그는 list로."""
      children = list(elem)
      if not children:
    ...
    list_elem = root.find(".//orderInfoList")
    if list_elem is not None:
      for order_elem in list_elem.findall("order"):
        order_dict = elem_to_dict(order_elem)
    ...
    logger.info(f"[11번가] 주문 {len(orders)}건 파싱 완료")
    return orders
  ...
  async def get_qna_list(
    ...
    11번가 셀러 API: GET /rest/prodservices/productQna
    qnaStatCd: 01=미답변, 02=답변완료
    """
    url = "https://api.11st.co.kr/rest/prodservices/productQna"
    headers = self._headers()
    ...
    logger.info("[11번가] GET /prodservices/productQna → %s", resp.status_code)
    logger.debug("[11번가] Q&A 응답: %s", resp.text[:800])

    if not resp.is_success:
      raise ElevenstApiError(f"Q&A 조회 실패: HTTP {resp.status_code}: {resp.text[:200]}")

    qna_list: list[dict[str, Any]] = []
    try:
      root = ET.fromstring(resp.text)
      for qna_el in root.findall(".//ProductQna"):
        qna: dict[str, Any] = {}
        for child in qna_el:
          qna[child.tag] = (child.text or "").strip()
        if qna:
          qna_list.append(qna)
    except ET.ParseError:
      logger.error("[11번가] Q&A XML 파싱 실패: %s", resp.text[:200])

    return qna_list

  async def reply_qna(self, qna_no: str, content: str) -> dict[str, Any]:
    """Q&A 답변 등록.

    11번가 셀러 API: POST /rest/prodservices/productQna/{qnaNo}
    """
    xml_body = (
      '<?xml version="1.0" encoding="UTF-8"?>\n'
      "<ProductQnaAnswer>\n"
      f"  <answerContent><![CDATA[{content}]]></answerContent>\n"
      "</ProductQnaAnswer>"
    )
    return await self._call_api("POST", f"/productQna/{qna_no}", body=xml_body)
  ...
  def transform_product(
    ...
    origin_raw = cfg.get("origin", "") or product.get("origin", "") or ""
...
def _build_elevenst_notice_xml(product: dict[str, Any]) -> str:
  ...
  caution = (
    ...
    or _ELEVENST_CAUTION_DEFAULTS.get(group, fallback)
  )
  ...
  code_value_map: dict[str, str] = {
    ...
    "23760437": fallback,                                       # A/S 책임자와 전화번호
