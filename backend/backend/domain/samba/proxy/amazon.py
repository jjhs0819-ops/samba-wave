...
class AmazonClient:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if not resp.is_success:
                ...
                raise AmazonApiError(
                    f"LWA 토큰 발급 실패 HTTP {resp.status_code}: {msg}"
                )
    ...
    def transform_product(
        ...
        """
        # SKU: 상품 고유 ID
        sku = str(product.get("id", "") or product.get("site_product_id", ""))[:40]
        ...
        if brand_name:
            attributes["brand_name"] = [
                {"value": brand_name, "marketplace_id": marketplace_id}
            ]
        ...
        if account_settings:
            if account_settings.get("fulfillment_channel"):
                attributes["fulfillment_availability"] = [
                    {
                        "fulfillment_channel_code": account_settings[
                            "fulfillment_channel"
                        ],
                        "quantity": 999,
                        "marketplace_id": marketplace_id,
    ...
    async def update_product(self, sku: str, payload: dict[str, Any]) -> dict[str, Any]:
