...
async def probe_source(site: str) -> ProbeResult:
  ...
  if not target:
    return ProbeResult(
      site=site, ok=False, error="probe 대상 미설정",
  ...
  try:
    if site == "MUSINSA":
      ...
      try:
        ...
        return ProbeResult(
          site=site, ok=False, latency_ms=elapsed,
      ...
      return ProbeResult(
        site=site, ok=len(missing) == 0,
        latency_ms=elapsed, missing_fields=missing,
        checked_at=datetime.now(timezone.utc).isoformat(),
      )
    else:
      return ProbeResult(
        site=site, ok=False, error="probe 미구현",
        checked_at=datetime.now(timezone.utc).isoformat(),
      )
...
async def probe_market(market_type: str, session: Any) -> ProbeResult:
  ...
  try:
    ...
    if not row or not row.value:
      elapsed = int((time.monotonic() - start) * 1000)
      return ProbeResult(
        site=market_type, ok=False, latency_ms=elapsed,
        error="설정 없음",
        checked_at=datetime.now(timezone.utc).isoformat(),
      )
    ...
    elapsed = int((time.monotonic() - start) * 1000)
    return ProbeResult(
      site=market_type, ok=ok, latency_ms=elapsed,
      error=error_msg,
      checked_at=datetime.now(timezone.utc).isoformat(),
    )
