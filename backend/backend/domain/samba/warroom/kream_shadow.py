...
def _trade_ok(kid: str, name: str) -> bool:
    ...
    # [2026-08-06] 거래이력 게이트 **해제**(KREAM_TRADE_GATE=1 이면 다시 켠다).
    # 비포켓몬 TCG(원피스·유희왕 등)를 거래≥1 로 막아 두니 재고가 있어도 등록이 안 됐다.
    #   실측(2026-08-06): 원피스 재고 138건 중 입찰 2건, 유희왕 11건 중 0건.
    #   같은 사이클 로그에도 '거래게이트 166' 으로 잡혔다.
    # 시세 불안정 우려는 남지만, 등록 판정이 이미 '최소하한으로 시장최저를 이기는가'로
    # 걸러내므로 못 이기는 건 어차피 안 들어간다.
    if os.environ.get("KREAM_TRADE_GATE") != "1":
        return True
