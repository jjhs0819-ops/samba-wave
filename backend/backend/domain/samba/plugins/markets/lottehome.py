...
def _transform_for_lottehome(
    ...
    for _m in _brand_mappings:
        ...
        if not _nm:
            continue
        # 1순위: brand 정확 매칭
        if _nm == _product_brand:
            _matched_brnd_no = _m["brnd_no"]
            break
        # 2순위: brand 포함 매칭
        if _nm in _product_brand or _product_brand in _nm:
            _matched_brnd_no = _m["brnd_no"]
            break
        # 3순위: 상품명에 브랜드명 포함
        if _nm in _product_name:
            _matched_brnd_no = _m["brnd_no"]
            break
    ...
    data: dict[str, Any] = {
        # 필수
        "brnd_no": product.get("brand_code") or _matched_brnd_no or creds.get("brnd_no", "010565"),
...
class LotteHomePlugin(MarketPlugin):
    ...
    async def execute(
        ...
        brand_mappings = policy.get("brandMappings") or store_lh.get("brandMappings") or []
        if brand_mappings:
