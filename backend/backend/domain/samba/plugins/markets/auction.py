...
def _split_multi_group_options(
    ...
    if dropped:
        ...
        logger.warning(
            f"[옥션] 옵션 {len(dropped)}건이 축 분할 실패로 제외됨 — "
...
class AuctionPlugin(MarketPlugin):
        ...
        try:
            ...
            if "상품이 없습니다" in err_msg or "not exist" in err_msg.lower():
                ...
                try:
                    ...
                    logger.warning(
                        f"[옥션] 상품 {master_no} PUT 실패했으나 GET 실존확인 → "
                        f"재등록 보류(매핑보존)"
                    )
                    return {
                        "success": False,
                    ...
                    logger.warning(
                        f"[옥션] 상품 {master_no} 없음(GET 404) → 신규등록 전환"
                    )
    ...
    async def _update_price_stock(
        ...
        def _fail_soft() -> dict[str, Any]:
            ...
            logger.warning(
                f"[옥션] sell-status 404 — 매핑 보존(fail-soft), 재시도 대기: "
        ...
        if has_options and any_sellable and cat_code:
            ...
            try:
                ...
                fb = await update_existing_freetext_stock(
                    client, master_no, options, site="auction", stock_cap=_cap
                )
                if fb.get("success"):
                    opt_msg = (
                    ...
                    if opt_result.get("success"):
                        ...
                        logger.warning(
                            f"[옥션] 옵션 재고 동기화 실패 — freetext: {fb.get('message')} "
