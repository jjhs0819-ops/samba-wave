...
class Cafe24Plugin(MarketPlugin):
  ...
  async def execute(
    ...
    if not mall_id:
      return {"success": False, "message": "카페24 Mall ID가 없습니다. 계정 설정에서 mallId를 입력해주세요."}
    if not client_id or not client_secret:
      return {"success": False, "message": "카페24 Client ID/Secret이 없습니다. 계정 설정에서 입력해주세요."}
    if not access_token and not refresh_token:
      return {"success": False, "message": "카페24 Access Token 또는 Refresh Token이 없습니다."}
    ...
    try:
      ...
      if not product_no:
        return {"success": False, "message": "카페24 상품 생성 실패: product_no 없음"}
      ...
      if category_id:
        try:
          ...
          logger.info(f"[카페24] 2단계 완료 — 카테고리 연결: category_no={category_id}")
      ...
      if options:
        opt_payload = Cafe24Client.build_options_payload(options, sale_price, max_stock)
        if opt_payload:
          try:
            await client.register_options(product_no, opt_payload)
            logger.info(f"[카페24] 3단계 완료 — 옵션 등록: {len(opt_payload)}개 옵션그룹")
            ...
            if variants:
              updates = Cafe24Client.build_variant_updates(
                options, variants, sale_price, max_stock,
              )
              for upd in updates:
                ...
                try:
                  ...
                  logger.warning(f"[카페24] variant {vcode} 업데이트 실패: {var_e}")
              logger.info(f"[카페24] 4단계 완료 — variant 재고 설정: {len(updates)}개")
