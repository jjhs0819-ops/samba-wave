...
_GS_KEYWORD_RULES: list[tuple[tuple[str, ...], str]] = [
    ...
    # ── 수영복 (실제 전시매장 있음: 시즌의류 > 수영복/래쉬가드 sectId 1719527) ──
    #    "수영팬츠"가 아래 하의(팬츠) 룰에 먼저 안 잡히게 신발 다음·하의 앞에 배치.
    (("래쉬가드", "래시가드", "라쉬가드"), "B43070907|1719527"),
    (
        ("수영복", "수영팬츠", "비키니", "모노키니", "스윔웨어", "비치웨어",
         "워터레깅스"),
        "B43070903|1719527",
    ),
    # ── 속옷/이너/잠옷/홈웨어 ──
    #   마놀 GS 계정에 "성인 속옷 전시매장"이 없음(키즈·유아·임부만 존재).
    #   A안(사용자 결정): 검증된 의류 전시매장(상의 B43071907|1662747 / 하의
    #   B43071903|1662742)에 매핑해 "등록은 되게" 한다. 진열 위치는 스포츠의류로
    #   잡히는 부정확 감수. (정확 진열 원하면 GS에 성인 속옷 전시매장 개설 필요)
    #   하의형(팬티/드로즈) → 하의, 나머지(브라·런닝·내의·잠옷·홈웨어 등) → 상의.
    (
        ("팬티", "드로즈", "트렁크팬티", "보정팬티", "사각팬티", "삼각팬티"),
        "B43071903|1662742",
    ),
    (
        ("브라렛", "브래지어", "브레이지어", "스포츠브라", "브라팬티", "브라 팬티", "캐미솔",
         "런닝", "러닝셔츠", "내의", "내복", "면속옷", "속옷", "언더웨어",
         "이너웨어", "란제리", "슬립", "보정속옷", "거들", "코르셋", "니퍼",
         "올인원", "보정", "잠옷", "파자마", "홈웨어", "이지웨어", "가운",
         "수면바지", "수면잠옷"),
        "B43071907|1662747",
    ),
