...
class SmartStorePlugin(MarketPlugin):
  ...
  async def execute(
    ...
    async def _try_send(d: dict[str, Any]) -> dict[str, Any]:
      if existing_no:
        try:
          logger.info(f"[스마트스토어] PUT 시도: origin={existing_no}, client_id={client_id[:8]}...")
          ...
          if "404" in str(e):
            ...
            if product_exists:
              ...
              for _wait in [10, 20]:
                logger.warning(f"[스마트스토어] 상품 {existing_no} PUT 404이지만 GET 성공 → {_wait}초 후 재시도")
            ...
            if skip_image or price_only:
              logger.warning(f"[스마트스토어] 수정 모드 상품 {existing_no} 404 → 신규등록 차단")
              return {
                "success": False,
            ...
            logger.warning(f"[스마트스토어] 상품 {existing_no} GET/PATCH 모두 404 → 신규등록 전환")
            try:
    ...
    try:
      unregistered_row = await self._get_setting(session, "smartstore_unregistered_tags")
