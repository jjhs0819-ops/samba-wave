...
def _similarity_match_smartstore(
    ...
    _SPECIFIC_ITEM_TOKENS: tuple[str, ...] = (
        ...
        "맨투맨",
        "니트",
        # 2026-07-14 보강 — "아동운동화"가 "주니어카시트"로 오매칭되던 것 방지
        # (둘 다 기존 목록에 없어 품목 불일치 패널티가 미적용됐음)
        "운동화",
        "러닝화",
        "스니커즈",
        "수영복",
    ...
    if best_leaf_match_len == 0:
        # 5단 트리 소싱처(더현대)는 소스 leaf가 스타일 수식어("데일리/하이브리드")고
        # 품목이 마지막-1 세그먼트("러닝/조깅/워킹화")에 옴 — 품목 세그먼트가
        # 후보 leaf에 매칭됐으면 신뢰, 아니면 기존대로 AI 위임 (2026-07-14)
        if len(seg_tokens_list) >= 2:
            _best_leaf = best.split(" > ")[-1].strip()
            # 유아동 가드: 소스에 유아동 키워드가 없는데 후보 경로에 있으면 거부
            # (top 전용 _RESTRICTED_TOPS 는 중분류 "아동신발" 등을 못 거름)
            _kids_kws = ("유아", "아동", "키즈", "주니어", "베이비", "신생아", "토들러")
            _src_has_kids = any(k in source_text for k in _kids_kws)
            _best_has_kids = any(k in best for k in _kids_kws)
            if (not _best_has_kids or _src_has_kids) and any(
                tok in _best_leaf for tok in seg_tokens_list[-2]
            ):
                return best
        return None
    return best
...
_SYNONYM_MAP: dict[str, list[str]] = {
    ...
    # 더현대Hi 어휘 (2026-07-14 유사도 매칭 실측 보강)
    "워킹화": ["워킹화", "운동화", "러닝화"],
    "러닝": ["러닝", "러닝화", "런닝"],
    "조깅": ["조깅", "러닝", "러닝화"],
    "역도": ["역도", "피트니스", "웨이트"],
    "피트니스화": ["피트니스화", "운동화", "트레이닝화"],
    "하계화": ["하계화", "샌들", "슬리퍼"],
    "슬라이드": ["슬라이드", "슬리퍼", "샌들"],
    "수영복": ["수영복", "스윔웨어", "래쉬가드"],
    "반신수영복": ["반신수영복", "수영복"],
    ...
    "소품": ["소품", "용품", "잡화"],
    "잡화": ["잡화", "용품", "소품", "액세서리"],
    # 연령군 — 마켓별 표기 차이 (출산/유아동, 키즈, 주니어 등)
    "유아동": ["유아동", "유아", "아동", "키즈", "주니어", "베이비", "신생아"],
    "유아": ["유아", "키즈", "아동", "베이비", "신생아", "주니어"],
    "아동": ["아동", "유아", "키즈", "주니어"],
    "키즈": ["키즈", "주니어", "아동", "유아", "베이비", "유아동"],
    "주니어": ["주니어", "키즈", "아동", "유아동"],
    "신생아": ["신생아", "유아", "베이비", "키즈"],
    "베이비": ["베이비", "신생아", "유아", "키즈"],
    "출산": ["출산", "유아동", "유아", "베이비"],
...
_KO_PRODUCT_TOKENS: tuple[str, ...] = (
    ...
    "부츠",
    "슬리퍼",
    # 더현대Hi leaf 합성어 (2026-07-14 유사도 매칭 실측 보강)
    "수영복",
    "래쉬가드",
    "비치웨어",
    "피트니스화",
...
MARKET_CATEGORIES: Dict[str, List[str]] = {
    ...
    # 패션플러스: 공식 카테고리 코드표 수령 전까지 시드를 넣지 않는다.
    # 시드 서비스는 병합만 하고 삭제하지 않아 창작값이 DB 에 영구 잔존하고,
    # 한글 경로는 전송 단계 숫자 게이트에 걸려 어차피 쓸 수 없다.
    "playauto": [
        # 플레이오토 자체 카테고리 체계
        "패션의류 > 남성의류 > 티셔츠",
        "패션의류 > 남성의류 > 팬츠",
