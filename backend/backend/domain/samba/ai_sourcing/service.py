...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
  "상의": [
    "티셔츠", "맨투맨", "후드", "니트", "셔츠", "블라우스", "가디건", "조끼",
    "반팔", "긴팔", "롱슬리브", "크루넥", "폴로", "스웨터", "후드티", "탱크탑",
    "크롭", "오버핏", "반팔티", "긴팔티",
  ],
  "아우터": [
    "자켓", "코트", "패딩", "점퍼", "바람막이", "야상", "블레이저", "후리스",
    "트랙탑", "블루종", "후드집업", "집업", "트레이닝자켓", "윈드브레이커",
    "가죽자켓", "데님자켓", "트러커", "헤링턴", "봄버",
  ],
  "바지": [
    "청바지", "슬랙스", "조거팬츠", "숏팬츠", "카고팬츠", "레깅스", "트레이닝팬츠",
    "와이드팬츠", "스웨트팬츠", "데님팬츠", "반바지", "숏츠", "치노",
    "밴딩팬츠", "원턱", "투턱",
  ],
  "원피스/스커트": ["원피스", "스커트", "미니스커트", "롱스커트", "미디스커트", "플리츠"],
  "가방": ["백팩", "크로스백", "토트백", "숄더백", "에코백", "클러치", "웨이스트백", "메신저백", "보스턴백"],
  "신발": [
    "운동화", "스니커즈", "슬리퍼", "샌들", "로퍼", "부츠", "구두", "런닝화",
    "발렛", "뮬", "슈즈", "트레킹화",
    "캔버스화", "데크슈즈", "첼시부츠", "워커",
  ],
  "시계/주얼리": ["시계", "반지", "목걸이", "팔찌", "귀걸이", "체인", "펜던트"],
  "패션소품": [
    "모자", "머플러", "장갑", "벨트", "양말", "넥타이", "스카프",
    "캡", "볼캡", "버킷햇", "비니", "삭스", "헤어밴드", "선글라스",
  ],
  "스포츠/레저": [
    "트레이닝복", "래쉬가드", "등산화", "요가복", "수영복",
    "트레이닝", "축구화", "농구화", "골프",
  ],
  "속옷/슬립웨어": ["속옷", "팬티", "브라", "파자마", "잠옷", "홈웨어"],
  "뷰티": ["틴트", "립스틱", "파운데이션", "향수", "쿠션", "아이섀도", "마스카라", "선크림"],
}

# 전체 카테고리 키워드 평탄화 (모듈 로드 시 1회만 계산)
_ALL_CATEGORY_KEYWORDS: list[str] = list(set(
...
BRAND_EN_TO_KR: dict[str, str] = {
  ...
  "puma": "푸마", "crocs": "크록스", "skechers": "스케쳐스",
  "vans": "반스", "converse": "컨버스", "reebok": "리복",
  "the north face": "노스페이스", "patagonia": "파타고니아",
  "columbia": "컬럼비아", "asics": "아식스", "mizuno": "미즈노",
  "salomon": "살로몬", "hoka": "호카", "on running": "온러닝",
  "gucci": "구찌", "louis vuitton": "루이비통", "chanel": "샤넬",
  "prada": "프라다", "celine": "셀린느", "balenciaga": "발렌시아가",
  "dior": "디올", "hermes": "에르메스", "burberry": "버버리",
  "coach": "코치", "michael kors": "마이클코어스",
  "spao": "스파오", "jeep": "지프", "snowpeak": "스노우피크",
  "casio": "카시오", "umbro": "엄브로", "champion": "챔피온",
  "tommy hilfiger": "타미힐피거", "calvin klein": "캘빈클라인",
  "lacoste": "라코스테", "polo": "폴로", "diesel": "디젤",
  "levis": "리바이스", "zara": "자라", "h&m": "H&M",
  "fila": "필라", "stussy": "스투시", "carhartt": "칼하트",
  "dickies": "디키즈", "kolon sport": "코오롱스포츠", "dynafit": "다이나핏",
  "noiago": "노이아고", "rawrow": "로우로우",
  "charles & keith": "찰스앤키스", "jansport": "잔스포츠",
  "descente": "데상트", "le coq": "르꼬끄", "k2": "K2",
  "blackyak": "블랙야크", "eider": "아이더", "nepa": "네파",
  "beanpole": "빈폴", "hazzys": "해지스", "mlb": "MLB",
  "sisley": "시슬리", "hugo boss": "휴고보스", "armani": "아르마니",
  "marc jacobs": "마크제이콥스", "tory burch": "토리버치",
...
class AISourcingService:
    ...
    for cat_name in categories:
      ...
      for search_kw in cat_keywords:
        try:
          ...
          logger.info(f"[AI소싱] 무신사 '{search_kw}'({cat_name}) → {len(items)}개 수집")
      ...
      if not brand and "texts" in item:
        ...
        for t in texts:
          if isinstance(t, str) and not t.isdigit() and not brand:
            brand = t.strip()
          elif isinstance(t, str) and not t.isdigit() and brand and not product_name:
  ...
  _EN_KW_MAP: dict[str, str] = {
    "track top": "트랙탑", "track jacket": "트랙탑", "track pants": "트레이닝팬츠",
    "sweat pants": "스웨트팬츠", "sweatpants": "스웨트팬츠",
    "hoodie": "후드", "hood": "후드", "zip up": "집업", "zip-up": "집업",
    "jacket": "자켓", "blazer": "블레이저", "coat": "코트",
    "t-shirt": "티셔츠", "tee": "티셔츠", "tshirt": "티셔츠",
    "shirts": "셔츠", "shirt": "셔츠", "polo": "폴로",
    "pants": "바지", "trousers": "슬랙스", "slacks": "슬랙스",
    "denim": "청바지", "jeans": "청바지",
    "shorts": "반바지", "short pants": "반바지",
    "sneakers": "스니커즈", "sneaker": "스니커즈",
    "running": "런닝화", "runner": "런닝화",
    "boots": "부츠", "boot": "부츠",
    "sandal": "샌들", "sandals": "샌들",
    "slipper": "슬리퍼", "slippers": "슬리퍼", "slide": "슬리퍼",
    "loafer": "로퍼", "loafers": "로퍼",
    "ballet": "발렛", "mule": "뮬",
    "backpack": "백팩", "bag": "가방", "tote": "토트백",
    "cap": "캡", "hat": "모자", "bucket": "버킷햇", "beanie": "비니",
    "socks": "양말", "belt": "벨트",
    "cardigan": "가디건", "knit": "니트", "sweater": "스웨터",
    "vest": "조끼", "windbreaker": "바람막이",
    "blouson": "블루종", "bomber": "봄버",
    "tint": "틴트", "lipstick": "립스틱",
  }
  ...
  async def fetch_naver_datalab(
    ...
    for cat_name in naver_categories:
      ...
      try:
        ...
        logger.info(f"[AI소싱] 네이버 데이터랩 {cat_name} → {len(keywords)}개 키워드")
      except Exception as e:
    ...
    for kw_info in all_keywords:
      ...
      product_keyword = self._extract_product_keyword(search_term, brand, all_cat_keywords)
  ...
  def _get_known_brand_list(self) -> list[str]:
    ...
    return [
      ...
      "푸마", "Puma", "크록스", "Crocs", "스케쳐스", "Skechers",
      "반스", "Vans", "컨버스", "Converse", "리복", "Reebok",
      "노스페이스", "The North Face", "파타고니아", "Patagonia",
      "컬럼비아", "Columbia", "아식스", "Asics", "미즈노", "Mizuno",
      "살로몬", "Salomon", "호카", "Hoka", "온러닝", "On Running",
      # 명품 (IP위험)
      "구찌", "Gucci", "루이비통", "Louis Vuitton", "샤넬", "Chanel",
      "프라다", "Prada", "셀린느", "Celine", "발렌시아가", "Balenciaga",
      "디올", "Dior", "에르메스", "Hermes", "버버리", "Burberry",
      "코치", "Coach", "마이클코어스", "Michael Kors",
      # 국내
      "무신사 스탠다드", "무신사스탠다드", "꼼파뇨", "디스커버리",
      "내셔널지오그래픽", "MLB", "폴햄", "탑텐", "스파오", "SPAO",
      "지프", "JEEP", "르무르", "스노우피크", "SNOWPEAK",
      "노르디스크", "엘브로", "카시오", "Casio", "다니엘웰링턴",
      "앤더슨벨", "커버낫", "이미스", "마르디메크르디",
      "엄브로", "Umbro", "챔피온", "Champion",
      "타미힐피거", "Tommy Hilfiger", "캘빈클라인", "Calvin Klein",
      "라코스테", "Lacoste", "폴로", "Polo",
      "디젤", "Diesel", "리바이스", "Levis",
      "지오다노", "유니클로", "자라", "ZARA", "H&M",
      "필라", "FILA", "휠라",
      "꼼데가르송", "아크네", "메종키츠네", "스투시", "Stussy",
      "칼하트", "Carhartt", "디키즈", "Dickies",
      # 국내 추가
      "코오롱스포츠", "KOLON SPORT", "코오롱", "다이나핏", "DYNAFIT",
      "노이아고", "NOIAGO", "로우로우", "RAWROW",
      "에고이스트", "써스데이아일랜드", "올리비아로렌", "제시뉴욕",
      "찰스앤키스", "Charles & Keith", "잔스포츠", "JanSport",
      "데상트", "DESCENTE", "르꼬끄", "Le Coq", "K2", "케이투",
      "블랙야크", "BLACKYAK", "아이더", "EIDER", "네파", "NEPA",
      "빈폴", "BEANPOLE", "해지스", "HAZZYS",
      "시슬리", "SISLEY", "마인", "MINE",
      "랩", "LAP", "에잇세컨즈", "8seconds",
      "쿠론", "COURONNE", "MCM", "닥스", "DAKS",
      "휴고보스", "Hugo Boss", "아르마니", "Armani",
      "마크제이콥스", "Marc Jacobs", "토리버치", "Tory Burch",
    ]

  def _extract_brand_from_keyword(
    self, keyword: str, known_brands: list[str]
  ) -> str:
    """검색어에서 브랜드명 추출."""
  ...
  def parse_sales_excel(
    ...
    for sheet_name in wb.sheetnames:
      ...
      for pat in sales_sheet_patterns:
        if pat.lower() in name_lower:
          ws = wb[sheet_name]
          logger.info(f"[AI소싱] 엑셀 시트 자동선택: '{sheet_name}' (패턴: {pat})")
          break
      if ws:
        break
      ...
      for sheet_name in wb.sheetnames:
        ...
        for row_idx in range(1, min(candidate.max_row or 10, 10) + 1):
          for col_idx in range(1, min(candidate.max_column or 30, 30) + 1):
            val = str(candidate.cell(row=row_idx, column=col_idx).value or "").strip()
            if val in ("상품명", "상품", "품명", "product"):
              ws = candidate
              logger.info(f"[AI소싱] 엑셀 시트 자동선택: '{sheet_name}' (헤더 발견)")
              break
          if ws:
            break
        if ws:
    ...
    logger.info(f"[AI소싱] 엑셀 시트: '{ws.title}', 총 {ws.max_row}행 × {ws.max_column}열")
    ...
    target_headers = {
      ...
      "날짜": ["날짜", "주문일", "결제일", "발주일", "주문일자", "결제일자", "date", "order_date"],
    }
    ...
    if month > 0 and "날짜" not in col_map:
      logger.warning(f"[AI소싱] 날짜 컬럼 없음 → 월 필터링 없이 전체 처리")
    ...
    for row_idx in range(header_row + 1, (ws.max_row or 0) + 1):
      product_name = str(ws.cell(row=row_idx, column=col_map["상품명"]).value or "").strip()
      if not product_name:
        continue
      total_count += 1
    ...
    if month > 0 and "날짜" in col_map:
      logger.info(f"[AI소싱] 엑셀 {month}월 필터: {total_count}행 중 {filtered_count}행 매칭")
    ...
    for brand, bd in brand_data.items():
      ...
      sales_weight = min(bd["total_sales"] / 100000, 10) if bd["total_sales"] > 0 else 1
  ...
  async def generate_combinations(
    ...
    valid_pairs = [
      p for p in brand_keyword_pairs
      if p.brand in all_brand_names
    ]
    ...
    for bs in safe_brands:
      ...
      if not cat_kws_for_brand:
        ...
        cat_kws_for_brand = ["운동화", "티셔츠", "반팔", "자켓", "바람막이", "슬리퍼"]
      for kw in cat_kws_for_brand:
        key = f"{bs.brand}|{kw}"
        if key not in seen_pairs:
          seen_pairs.add(key)
          unique_pairs.append(BrandKeywordPair(
            brand=bs.brand, keyword=kw, source=bs.source, count=1, score=bs.score,
          ))
    ...
    async def _fetch_count(pair: BrandKeywordPair):
      async with semaphore:
        try:
          ...
          result = await self.musinsa.search_products(
            keyword=search_keyword, sort="POPULAR", size=1,
          )
          return pair, result.get("totalCount", 0), search_keyword
        except Exception as e:
          logger.warning(f"[AI소싱] 조합 조회 실패: {pair.brand}×{pair.keyword}: {e}")
          return pair, 0, ""
  ...
  async def run_analysis(
    ...
    # 1) 무신사
    if use_musinsa:
      musinsa_brands, musinsa_pairs = await self.fetch_musinsa_popular(categories=m_cats)
