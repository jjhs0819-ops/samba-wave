...
_DELETE_GHOST_SIGNALS = (
    ...
    "판매종료 된 상품",
    "판매중지된 상품",
    # 11번가: 이미 판매금지된 상품은 판매중지 API 자체가 거부 → ghost 정리 대상 (#666)
    "판매 금지된 상품",
    # 롯데ON 7777: "판매중지가 된 상품" — 조사 차이로 위 "판매중지된 상품"에 안 걸림 (#666)
    "판매중지가 된 상품",
    "현재 판매 중인 상품이 아닙니다",
    "유효하지 않은 상품",
    "상품을 찾을 수 없",
    # 롯데홈쇼핑 영구중단(sale_stat_cd=30) 재삭제 — 이미 마켓에서 내려간 상태이므로
    # ghost 자동정리 대상(거부 아님). 코드 "0011"은 광범위 오매칭 위험이라 전체 문구로 매칭. (#405)
    "영구중단된 상품은 수정할 수 없습니다",
    "8888",
    "9999",
    "HTTP 404",
)
...
async def _delete_lottehome(
    ...
    """롯데홈쇼핑 상품 삭제 (영구중단)."""
    # MD 승인 대기 중인 상품은 skipMdApproval=False일 때만 삭제 차단
    if account:
