...
_autotune_batch_started_at: dict[
    tuple[str, str], float
] = {}  # 현재 배치 refresh_products_bulk 시작 시각
            ...
            try:
                ...
                async with get_write_session() as session:
                    ...
                    if products:
                        ...
                        async def _on_result(product, r, idx=0, total=0):
                            ...
                            async with _session_lock:
                                ...
                                for acc_id in reg_accounts:
                                    ...
                                    if _acc_items:
                                        # 롯데홈쇼핑 MD 승인 대기 체크 — skipMdApproval=False인 경우만
                                        if market_type == "lottehome":
                                            _lh_acc_obj = _account_cache.get(acc_id)
                                            _lh_extra = (
                                                getattr(
                                                    _lh_acc_obj,
                                                    "additional_fields",
                                            ...
                                            if not _lh_extra.get("skipMdApproval"):
                                                _qa_status = _m_nos.get(
                                                    f"{acc_id}_qa", ""
                                                )
                        ...
                        _autotune_batch_started_at[_gkey] = _bulk_start_ts
                        ...
                        try:
                            ...
                            pass
                        # 배치 완료 직후 heartbeat 갱신 — 배치간 inter-batch 공백(SELECT/포스트처리) 동안
                        # heartbeat 가 수백 초 멈추는 것을 방지
                        _pc_hb(device_id)[site] = time.time()
...
async def autotune_active_cycles():
    ...
    for dev, site_tasks in list(_pc_site_tasks.items()):
        ...
        for site, task in list(site_tasks.items()):
            ...
            batch_start = _autotune_batch_started_at.get(gkey, 0)
            batch_started_ago_sec = int(now_ts - batch_start) if batch_start else None
            ...
            cycles.append(
                {
                    ...
                    "batch_started_ago_sec": batch_started_ago_sec,
                }
            )
