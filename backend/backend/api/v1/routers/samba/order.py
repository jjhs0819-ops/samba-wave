...
def _parse_lotteon_order(
  ...
  return {
    ...
    "account_label": label,
    "source": "lotteon",
    # 롯데ON: odNo = 주문번호 (getSROrderList 응답 기준)
    "order_number": str(item.get("odNo", "") or item.get("orderNo", "")),
    "shipment_id": str(item.get("odNo", "") or item.get("orderNo", "")),
    "product_id": str(item.get("pdNo", "") or item.get("spdNo", "") or ""),
    "product_name": item.get("pdNm", "") or item.get("goodsNm", "") or "",
    "product_option": item.get("optionName", "") or item.get("sitmNm", "") or "",
    "quantity": int(item.get("ordQty", 1) or 1),
    "sale_price": int(item.get("saleAmt", 0) or item.get("slPrc", 0) or 0),
    "cost": 0,
    "status": status,
    "customer_name": item.get("rcvrNm", "") or item.get("buyerNm", "") or "",
    "customer_phone": item.get("rcvrTelNo", "") or item.get("rcvrMobileNo", "") or "",
