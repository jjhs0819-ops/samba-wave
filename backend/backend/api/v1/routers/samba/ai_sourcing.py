...
async def get_categories():
  ...
  return {
    ...
    "naver": [
      {"id": cid, "name": name}
      for name, cid in NAVER_DL_CATEGORIES.items()
    ],
  }
...
async def analyze_full(
  ...
  async def stream():
    try:
      ...
      yield _sse("log", f"[설정] {main_category} / {month_label} / 목표 {target_count:,}개")
      ...
      try:
        ...
        yield _sse("log", f"[확장앱] 랭킹: {len(ranking_items)}개 상품 수집 완료")
        if ranking_items:
          for it in ranking_items[:3]:
            yield _sse("log", f"  [{it.get('rank')}] {it.get('brand', '')} - {it.get('name', '')[:40]}")
      except _asyncio.TimeoutError:
        yield _sse("log", "[확장앱] 랭킹 타임아웃 (확장앱 미연결?) — 기존 방식으로 대체")
      finally:
      ...
      try:
        ...
        yield _sse("log", f"[확장앱] 검색 키워드: {len(search_keywords_raw)}개 수집 완료")
      ...
      if ranking_items:
        musinsa_brands, musinsa_pairs = svc.extract_brands_from_ranking(ranking_items)
        yield _sse("log", f"랭킹에서 {len(musinsa_brands)}개 브랜드, {len(musinsa_pairs)}개 쌍 추출")
      else:
        musinsa_brands, musinsa_pairs = await svc.fetch_musinsa_popular(categories=m_cats)
        yield _sse("log", f"API 검색으로 {len(musinsa_brands)}개 브랜드, {len(musinsa_pairs)}개 쌍 추출")
            ...
            if b.lower() in kw_lower:
              ...
              for cat_kw in _sorted_kws:
                if cat_kw.lower().replace(" ", "") in remaining_lower:
                  # musinsa_pairs에 추가
                  musinsa_pairs.append(BrandKeywordPair(
                    brand=b, keyword=cat_kw, count=1,
                    score=max(501 - rank, 1), source="search_keyword",
                  ))
                  kw_pairs_added += 1
                  break
              break
        if kw_pairs_added:
          yield _sse("log", f"검색어에서 {kw_pairs_added}개 브랜드×키워드 쌍 추가 추출")
      ...
      try:
        ...
        yield _sse("log", f"네이버 {len(naver_brands)}개 브랜드, {len(naver_pairs)}개 브랜드×키워드 쌍 추출")
        if ntop5:
      ...
      if excel_content:
        ...
        yield _sse("log", f"엑셀 {len(excel_brands)}개 브랜드 추출 (TOP5: {', '.join(etop5)})")
      ...
      yield _sse("log", f"총 {len(sorted_brands)}개 → 안전 {len(safe_brands)}개 / 위험 {unsafe}개")
      ...
      yield _sse("log", f"완료: {len(combinations)}개 그룹, 예상 {total_est:,}개 상품")
...
async def receive_ranking_data(request: dict[str, Any]):
  ...
  from backend.domain.samba.ai_sourcing.service import (
    BrandScore, BrandKeywordPair, CATEGORY_KEYWORDS,
  )
  ...
  for item in ranking_items:
    ...
    if brand not in brand_map:
      brand_map[brand] = {"count": 0, "total_sales": 0, "categories": [], "keywords": []}
    bm = brand_map[brand]
    bm["count"] += 1
    ...
    for kw in found_keywords:
      ...
      if pair_key not in pair_map:
        pair_map[pair_key] = {"brand": brand, "keyword": kw, "count": 0, "score": 0}
        ...
        if b.lower() in kw_lower:
          ...
          for cat_kw in sorted(all_cat_keywords, key=len, reverse=True):
            if cat_kw.lower().replace(" ", "") in remaining_lower:
              pair_key = f"{b}|{cat_kw}"
              if pair_key not in pair_map:
                pair_map[pair_key] = {"brand": b, "keyword": cat_kw, "count": 0, "score": 0}
              pair_map[pair_key]["count"] += 1
...
async def receive_collect_result(request: dict[str, Any]):
  ...
  if error:
    ...
    logger.info(f"[AI소싱] 랭킹: {len(items)}개 상품, 링크={debug.get('productLinks', 0)}")
    for it in items[:5]:
      logger.info(f"  상품: [{it.get('rank')}] {it.get('brand', '')} - {it.get('name', '')[:40]} ₩{it.get('price', 0)}")
    if not items:
