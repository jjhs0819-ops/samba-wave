...
def upgrade() -> None:
    ...
    # 교환 상품 회수 추적 (수기 입력)
    op.add_column(
        "samba_return", sa.Column("exchange_retrieval_status", sa.Text(), nullable=True)
    )
    op.add_column(
        "samba_return",
        sa.Column("exchange_retrieved_at", sa.DateTime(timezone=True), nullable=True),
    )

    # 소싱처 재출고 정보 (수기 입력)
    op.add_column(
        "samba_return", sa.Column("exchange_reship_company", sa.Text(), nullable=True)
    )
    op.add_column(
        "samba_return", sa.Column("exchange_reship_tracking", sa.Text(), nullable=True)
    )

    # 고객 도착 일자 (수기 입력)
    op.add_column(
        "samba_return",
        sa.Column("exchange_delivered_at", sa.DateTime(timezone=True), nullable=True),
    )
