...
def upgrade() -> None:
    ...
    # 신규 컬럼
    op.add_column(
        "samba_search_filter", sa.Column("parent_id", sa.Text(), nullable=True)
    )
    op.add_column(
        "samba_search_filter",
    ...
    op.create_index(
        op.f("ix_samba_search_filter_parent_id"),
        "samba_search_filter",
        ["parent_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    # 폴더 삭제
    op.get_bind().execute(
        sa.text("DELETE FROM samba_search_filter WHERE is_folder = true")
    )
    op.drop_index(
        op.f("ix_samba_search_filter_parent_id"), table_name="samba_search_filter"
    )
