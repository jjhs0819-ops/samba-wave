...
def test_thehyundai_composite_cat1_lejyeo_sports() -> None:
    ...
    assert (
        detect_notice_group(
            _th_product("레저/스포츠 > 스포츠 슈즈 > 여성스포츠화 > 러닝/조깅/워킹화")
        )
        == "shoes"
    )
    assert (
        detect_notice_group(
            _th_product("레저/스포츠 > 일반스포츠 > 남성/공용의류 > 하의")
        )
        == "wear"
    )
    assert (
        detect_notice_group(_th_product("레저/스포츠 > 워터 > 실내수영복 > 반신수영복"))
        == "wear"
    )
...
def test_thehyundai_exact_sports_not_hijacking_items() -> None:
    ...
    assert (
        detect_notice_group(_th_product("레저/스포츠 > 골프 > 모자/헤어밴드 > 볼캡"))
        == "accessories"
    )
    assert (
        detect_notice_group(_th_product("레저/스포츠 > 골프 > 골프백/캐디백 > 캐디백"))
        == "bag"
    )
    assert (
        detect_notice_group(_th_product("레저/스포츠 > 골프 > 골프화 > 스파이크"))
        == "shoes"
    )
    assert (
        detect_notice_group(_th_product("레저/스포츠 > 데일리 > 하계화 > 슬리퍼/샌들"))
        == "shoes"
    )
    # 진짜 장비는 sports 유지 (SSG 에서 의류 고시로 안전 폴백)
    assert (
        detect_notice_group(
            _th_product("레저/스포츠 > 캠핑/아웃도어 > 숙영장비 > 텐트/타프/그늘막")
        )
        == "sports"
    )
...
def test_thehyundai_cat4_exact_and_vocab() -> None:
    ...
    assert (
        detect_notice_group(_th_product("패션 > 워치/쥬얼리 > 목걸이")) == "accessories"
    )
    assert (
        detect_notice_group(_th_product("유아동/패밀리 > 토들러패션 > 바지/레깅스"))
        == "wear"
    )
    assert (
        detect_notice_group(_th_product("유아동/패밀리 > 유아패션 > 우주복")) == "wear"
    )
    assert (
        detect_notice_group(
            _th_product("레저/스포츠 > 워터 > 수상레저/서핑/스노클링 > 수모/수경")
        )
        == "accessories"
    )
...
def test_build_ssg_notice_caution_from_care_instructions() -> None:
    ...
    _, attrs = build_ssg_notice(
        {
            **_th_product("레저/스포츠 > 스포츠 슈즈 > 여성스포츠화"),
            "care_instructions": long_care,
        }
    )
