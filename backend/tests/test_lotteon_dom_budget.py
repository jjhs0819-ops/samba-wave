...
class TestProductTimeoutHelper:
    ...
    def test_lotteon_returns_150(self) -> None:
        # 실측 90s 적용 시 큐 적체로 timeout 다수 발생 → 150s 복원
        from backend.domain.samba.collector.refresher import get_product_timeout

        assert get_product_timeout("LOTTEON") == 150

    def test_ssg_returns_150(self) -> None:
        # 큐 대기 흡수 마진 확대
        from backend.domain.samba.collector.refresher import get_product_timeout

        assert get_product_timeout("SSG") == 150
...
class TestSitePtoMapping:
    ...
    def test_lotteon_in_map(self) -> None:
        ...
        assert SITE_PRODUCT_TIMEOUT["LOTTEON"] == 150

    def test_ssg_in_map(self) -> None:
        from backend.domain.samba.collector.refresher import SITE_PRODUCT_TIMEOUT

        assert SITE_PRODUCT_TIMEOUT["SSG"] == 150
